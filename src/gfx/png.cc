...
namespace gh
{
  ...
  void free(void*p) {
	...
	bool ReadFromRaw(Png *png,const Raw *in)
	{
		...
		for(;;)
		{
			...
			else if(memmem("IHDR",p+4,4))
			{
				...
				png->image_data=(uint8_t*)new char[idat_size];
				temp=(uint8_t*)new char[temp_size];
	...
	bool WriteFromPng(Raw *out,const Png *in)
	{
		...
		uint8_t *data=(uint8_t*)new char[in->image_header.height*in->image_header.width*bw/8+in->image_header.height];
		...
		uint8_t *temp=(uint8_t*)new char[reserv];
		uint8_t *p=temp;
		...
		out->data=(uint8_t*)new char[out->size];
	...
	{
		uint8_t *frgb=(uint8_t*)new char[width*height*3+height];
		for(uint32_t y=0;y<height;y++)
		{
			//frgb[y+y*width*3]=0;
			//memcpy(frgb+1+y+y*width*3,rgb+y*width*3,width*3);
	...
	{
		uint8_t *frgba=(uint8_t*)new char[width*height*4+height];
		for(uint32_t y=0;y<height;y++)
		{
	...
	{
		return gh::GetGrayPixel[image_header.color_depth](this,x,y);
	}
}
