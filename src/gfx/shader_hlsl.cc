...
shader::Shader* d3d9::GraphicsD3D::CreateShader(shader::ShaderType st)
{
  ...
  case ClassID_SoftLight:
    return new SoftLightBody();
  ...
  case ClassID_ShadowMap:
    return new ShadowMapBody();
  case ClassID_ShadowMapProjection:
    return new ShadowMapProjectionBody();
  case ClassID_GrassMap:
    return new GrassMapBody();
  case ClassID_GrassProjector:
    return new GrassProjectorBody();
  }
  assert(false);
  return NULL;
}
...
void SpecularLightBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  ...
  _(main) << "specular.xyz=specular.xyz+specularlight_color.xyz*pow(max(0,dot(normal,normalize(normalize(camera_position.xyz-position.xyz)+directionallight_direction))),specularlight_color.w);\n"
    ;
...
void SoftLightBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(constant) << "float3 directionallight_direction:register(c" << (used_constant + 0) << ");\n"
    "float4 directionallight_color:register(c" << (used_constant + 1) << ");\n"
    "float4 specularlight_color:register(c" << (used_constant + 2) << ");\n"
    "float4 ambientlight_color:register(c" << (used_constant + 3) << ");\n";
  used_constant += 4;

  if (format & FTX_NORMAL) {
    _(main) <<
      "float bright=max(0,dot(normal,directionallight_direction));\n"
      "diffuse.xyz=diffuse.xyz*(directionallight_color.xyz*bright+ambientlight_color*(1-bright));\n"
      "specular.xyz=specular.xyz+specularlight_color.xyz*pow(max(0,dot(normal,normalize(normalize(camera_position.xyz-position.xyz)+directionallight_direction))),specularlight_color.w);\n"
      ;
  }
  else {
    _(main) << "diffuse.xyz=ambientlight_color.xyz;\n";
  }
}

int SoftLightBody::PipeType(int format)
{
  return FTX_DIFFUSE | FTX_SPECULAR;
}

void SoftLightBody::Pixel(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(main) << "color.xyz=color.xyz+specular.xyz;\n"
    "color.xyz=min(color.xyz,float3(1,1,1));\n"
    ;
...
void SoftLightBody::SetupConstant(ShaderConstant& sc)
{
  LPDIRECT3DDEVICE9 dev = sc.dd->RawDevice();
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 0, direction_.v, 1);
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 1, diffuse_.ToArray(), 1);
  specular_.a = power_;
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 2, specular_.ToArray(), 1);
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 3, ambient_.ToArray(), 1);
}
...
ShadowMapBody::ShadowMapBody()
  :uv_(0)
{
}

void ShadowMapBody::SetTexture(Texture* t)
{
  depth_ = t;
}

void ShadowMapBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(constant) << "float4x4 mat_view_projection:register(c" << (used_constant + 0) << ");\n";
  used_constant += 4;
...
int ShadowMapBody::PipeType(int format)
{
  return FTX_TEX1;
}

void ShadowMapBody::Pixel(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(main) << "color.r=uv0.x/uv0.y;\n";
}

void ShadowMapBody::SetupConstant(ShaderConstant& sc)
{
  LPDIRECT3DDEVICE9 dev = sc.dd->RawDevice();
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 0, MatrixTranspose(projection_), 4);
}

//////////////////////////////////////
// ShadowMapProjection

ShadowMapProjectionBody::ShadowMapProjectionBody()
  :uv_(0)
{
}

void ShadowMapProjectionBody::SetTexture(Texture* t)
{
  depth_ = t;
}

void ShadowMapProjectionBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(constant) << "float4x4 mat_shadow_projection:register(c" << (used_constant + 0) << ");\n";
  used_constant += 4;

  _(main) << "float4 smp=mul(position,mat_shadow_projection);\n"
    //"float theta = atan2(smp.y, smp.x);\n"
    //"float radius = length(smp.xy);\n"
    //"radius = pow(radius, 0.2);\n"
    //"smp.x = radius * cos(theta);\n"
    //"smp.y = radius * sin(theta);\n"
    "uv0.x=(smp.x+1.0)/2.0;\n"
    "uv0.y=(-smp.y+1.0)/2.0;\n"
    "uv0.z=smp.z;\n"
    "uv0.w=smp.w;\n"
...
int ShadowMapProjectionBody::PipeType(int format)
{
  return FTX_TEX1 | FTX_FLOAT4_TEX1;
}

void ShadowMapProjectionBody::Pixel(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  sampler_ = used_sampler++;
  ...
  _(constant) << "Texture = <tex" << sampler_ << ">;\n"
    "};\n";
  _(main) <<
    "float2 suv=float2(uv0.x,uv0.y);"
    "float R=1.0/256.0;"
    "float2 kern[12];"
    "for(int i=0;i<12;i++){"
    "kern[i]=float2(cos(6.28/12*i)*R,sin(6.28/12*i)*R);"
    "}"
    "float distance=0;"
    "float depth;"
    "for(int i=0;i<12;i++){"
    "depth=tex2D(samp" << sampler_ << ",suv+kern[i].xy).r;\n"
    "if(uv0.z/uv0.w>depth+0.001){"
    "distance+=(uv0.z/uv0.w-depth);"
    "}"
    "}"
    //"distance*=0.25;"
    "float dark=0;"
    "for(int j=0;j<12;j++){"
    "depth=tex2D(samp" << sampler_ << ",suv+kern[j].xy*distance).r;\n"
    "if(uv0.z/uv0.w>depth+0.001){"
    "dark+=0.08;"
    "}"
    "}"
    "color=(1-dark*0.5)*color;"
    ;
}
//https://http.download.nvidia.com/developer/presentations/2005/SIGGRAPH/Percentage_Closer_Soft_Shadows.pdf

void ShadowMapProjectionBody::SetupConstant(ShaderConstant& sc)
{
  LPDIRECT3DDEVICE9 dev = sc.dd->RawDevice();
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 0, MatrixTranspose(projection_), 4);
  sc.dd->SetTexture(depth_, sampler_);
  dev->SetSamplerState(sampler_, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
  dev->SetSamplerState(sampler_, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
  dev->SetSamplerState(sampler_, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
  dev->SetSamplerState(sampler_, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
}

//////////////////////////////////////
// GrassMap

GrassMapBody::GrassMapBody()
  :uv_(0)
{
}

void GrassMapBody::SetTexture(Texture* t)
{
  depth_ = t;
}

void GrassMapBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(constant) << "float4x4 mat_view_projection:register(c" << (used_constant + 0) << ");\n";
  used_constant += 4;
...
int GrassMapBody::PipeType(int format)
{
  return FTX_TEX1|FTX_NORMAL;
}

void GrassMapBody::Pixel(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(main) << "color.r=uv0.x/uv0.y;\n"
    "color.g=normal.x;\n"
    "color.b=normal.z;\n"
...
void GrassMapBody::SetupConstant(ShaderConstant& sc)
{
  LPDIRECT3DDEVICE9 dev = sc.dd->RawDevice();
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 0, MatrixTranspose(projection_), 4);
}
...
GrassProjectorBody::GrassProjectorBody()
  :uv_(0)
{
}

void GrassProjectorBody::SetTexture(Texture* t)
{
  depth_ = t;
}

void GrassProjectorBody::Vertex(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(constant) << "float4x4 mat_view_projection:register(c" << (used_constant + 0) << ");\n";
  used_constant += 4;
...
int GrassProjectorBody::PipeType(int format)
{
  return FTX_TEX1 | FTX_NORMAL;
}

void GrassProjectorBody::Pixel(int format, std::string& constant, int& used_constant, int& used_sampler, std::string& main)
{
  _(main) << "color.r=uv0.x/uv0.y;\n"
    "color.g=normal.x;\n"
    "color.b=normal.z;\n"
...
void GrassProjectorBody::SetupConstant(ShaderConstant& sc)
{
  LPDIRECT3DDEVICE9 dev = sc.dd->RawDevice();
  dev->SetVertexShaderConstantF(sc.vertex_register_offset + 0, MatrixTranspose(projection_), 4);
}
