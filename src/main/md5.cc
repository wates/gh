...
{
    MD5_CTX *ctx=(MD5_CTX*)context_;
    MD5Update(ctx,(unsigned char*)buffer,length);
}

void MD5::Final()
{
    MD5_CTX *ctx=(MD5_CTX*)context_;
    MD5Final(result_,ctx);
}

const uint8_t* MD5::Result()const
{
    return result_;
}
...
const char* MD5::Hex()
{
    ...
    for(int i=0;i<16;i++)
    {
        hex_[i*2]=h[(result_[i]>>4)];
        hex_[i*2+1]=h[(result_[i]&0xf)];
    }
    hex_[32]=0;
    return hex_;
}
